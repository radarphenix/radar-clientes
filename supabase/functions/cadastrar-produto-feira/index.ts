...
const decimal3 = (v:string) => /^\d{1,9},\d{3}$/.test(v)
const inteiro = (v:string) => /^\d{1,9}$/.test(v)
...
Deno.serve(async req => {
  ...
  try {
    ...
    const dados={empresa:texto(b.empresa,160),contato:texto(b.contato,160),telefone:texto(b.telefone,30),email:texto(b.email,254).toLowerCase(),responsavel:texto(b.responsavel,160),maquina:texto(b.maquina,160),tipo_papel:papel,produto,modelo:modelo||null,posicao:posicao||null,comprimento:texto(b.comprimento,30),largura:texto(b.largura,30),cfm:texto(b.cfm,80),gramatura:texto(b.gramatura,80),espessura:texto(b.espessura,80),teflonada:produto==='Secadora Espiral'&&b.teflonada===true,durabilidade:texto(b.durabilidade,160),velocidade_maquina:texto(b.velocidade_maquina,160),informacoes_adicionais:texto(b.informacoes_adicionais,5000)}
    if(!dados.empresa||!dados.contato||!dados.telefone||!dados.email||!dados.responsavel||!dados.maquina||!dados.comprimento||!dados.largura||!dados.cfm||!dados.gramatura||!dados.espessura||!dados.durabilidade||!dados.velocidade_maquina)return json({ok:false,mensagem:'Preencha os campos obrigatórios.'},400)
    ...
    const opcoesModelo=modelos(papel,produto)
    if((opcoesModelo.length&&!opcoesModelo.includes(modelo))||(!opcoesModelo.length&&modelo))return json({ok:false,mensagem:'Modelo inválido para o produto informado.'},400)
